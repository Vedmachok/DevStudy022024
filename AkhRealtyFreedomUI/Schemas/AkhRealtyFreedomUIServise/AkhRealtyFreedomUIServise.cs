...
public decimal GetTotalAmountByTypeId(string realtyTypeId, string realtyOfferTypeId,
			string entityName)
		{
			...
			Select select = new Select(UserConnection)
				.Column(Func.Sum("AkhPrice"))
				.From(entityName)
				.Where("AkhTypeId").IsEqual(Column.Parameter(new Guid(realtyTypeId)))
				.And("AkhOfferTypeId").IsEqual(Column.Parameter(new Guid(realtyOfferTypeId)))
				as Select;
        ...
        {
			return "Server responded with: pong!";
        }
